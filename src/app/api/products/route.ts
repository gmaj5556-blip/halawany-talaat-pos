...
import { getDb, initializePgSchema } from '@/lib/db';

export async function GET(request: NextRequest) {
  try {
    // Initialize schema if on cloud (only runs once)
  ...
  try {
    ...
    if (db.type === 'pg') {
      const res = await db.prepare(
        'INSERT INTO products (category_id, name, name_ar, price, cost, track_stock, current_stock, unit) VALUES (?, ?, ?, ?, ?, ?, ?, ?) RETURNING *'
